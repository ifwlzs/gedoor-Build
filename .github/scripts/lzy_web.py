...
# 请求头
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.104.5112.81 Safari/537.36 Edg/99.104.1293.54',
...
def upload_file(file_dir, folder_id):
    ...
    res = requests.post(url_upload, data=post_data, files=files, headers=headers, cookies=cookie, timeout=600,
                        verify=True).json()
...
def upload_folder(folder_dir, folder_id):
    ...
    for file in file_list:
        ...
        if path.endswith(".apk") or path.endswith(".zip") or path.endswith(".rar"):
            print("path: ",path)
